...
function StepIndicator({ step, total }: { step: number; total: number }) {
  return (
    <div className="flex items-center gap-1.5 mb-8" role="group" aria-label={`Step ${step} of ${total}`}>
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{ backgroundColor: "var(--brand-wa)", color: "var(--charcoal)" }}
      >
...
export default function OfferingQualifier({ isOpen, onClose }: OfferingQualifierProps) {
  ...
  // Focus the close button once, on initial open only.
  useEffect(() => {
    if (isOpen) {
  ...
  // On each subsequent step, move focus to the new question instead of
  // stealing it back to Close — keeps keyboard flow moving forward.
  useEffect(() => {
    if (isOpen && step > 0) {
                    ...
                    <AnimatePresence mode="wait" initial={false}>
                      ...
                      >
                        <h2
                          ref={stepHeadingRef}
                          tabIndex={-1}
                          className="font-display font-light text-[var(--charcoal)] mb-6 focus:outline-none"
                          style={{ fontSize: "var(--text-section)" }}
                        >
                          {steps[step].question}
                        </h2>
